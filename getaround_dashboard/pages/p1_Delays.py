...
st.sidebar.markdown("""
    ...
    * [Graph 3](#plot-3) - Rate of cancellation
...
with col1:
    ...
    fig1 = px.pie(dataset_delay, names="checkin_type", hole=.5, width=500)
...
with col2:
    ...
    fig4 = px.pie(dataset_delay.loc[dataset_delay.state=="ended",:], names="delay_status", hole=.5, width=500)
    fig4.update_traces(textposition="inside", textinfo="percent+label")
    fig4.update_layout(showlegend=False)
    st.plotly_chart(fig4, use_container_width=True)
    st.markdown("""
        More than 52% of cars are returned with delay which
        is the very high rate.
...
with col2:
    ...
    fig4 = px.pie(dataset_delay, names="previous_ended_rental_id_category", hole=.5, width=500)
    fig4.update_traces(textposition="inside", textinfo="percent+label")
    fig4.update_layout(showlegend=False)
    st.plotly_chart(fig4, use_container_width=True)
    st.markdown("""
...
fig5 = px.histogram(dataset_delay,
                    ...
                    width=500,
                    labels={"delay_status" : "delay status", "previous_ended_rental_id_category" : "Car rented previously"})
...
fig5 = px.histogram(dataset_delay, x= "delay_status", color="checkin_type", width=500, labels={"delay_status" : "delay status", "checkin_type" : "checkin type"})
st.plotly_chart(fig5, use_container_width=True)
st.markdown("""
    Proportionately speaking, people who rented a car 
    using the connect way seems to return it on time or in advance
    more frequently.
